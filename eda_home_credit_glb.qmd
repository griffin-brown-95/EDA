...
sapply(app_train,class) %>% 
  table()
```
What we are looking at is a dataset with 307,508 rows and 122 columns. There are 16 character columns, and 106 numeric.
...
The question remains, how would we like to approach these two variables. As a pensioner, maybe they never have worked, so perhaps 0 would work as a replacement to 365243. This can easily be changed with further context. As I mentioned before, I will be taking the top outliers for AMT_TOTAL_INCOME out of the dataset completely.
...
combined_proportions <- bind_rows(proportions_list) %>% 
  mutate(default_target = ifelse(TARGET == 0, target_0, target_1),
         threshold = abs(Proportion - default_target))
...
```{r}
for(cat_var in categorical_vars) {
  proportions_list[[cat_var]] <- proportions_list[[cat_var]] %>%
    left_join(overall_target_prop, by = "TARGET") %>%
    # Add the default_target column by comparing if proportion > perc
    mutate(default_target = ifelse(Proportion > perc, "Above", "Below"))
}

# Combine all data frames into one
final_table <- bind_rows(proportions_list, .id = "Variable")
...
# Define a dataframe to accumulate results
combined_proportions <- tibble()

# Loop through categorical variables
for(cat_var in categorical_vars) {
  # Compute proportions for each category within the variable
  proportions <- character_app_train %>%
    count(!!sym(cat_var), TARGET, name = "n") %>%
    group_by(!!sym(cat_var)) %>%
    mutate(Proportion = n / sum(n)) %>%
  ...
  # Bind the current proportions dataframe to the combined result
  combined_proportions <- bind_rows(combined_proportions, proportions)
}

# Print the combined dataframe
combined_proportions %>% 
  mutate(default_target = ifelse(TARGET = 0, ))
